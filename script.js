...
function readFile(file) {
  ...
  reader.onload = function(e) {
    ...
    document.getElementById('drop-area').textContent = 'File loaded. Ready to generate NBT.';
  };
...
function readExtractFile(file) {
  ...
  reader.onload = function(e) {
    ...
    document.getElementById('extract-drop-area').textContent = 'File loaded. Ready to extract commands.';
  };
  reader.readAsText(file);
}

// Extract Commands Event Listeners
...
extractDropArea.addEventListener('click', () => {
  document.getElementById('extract-input-file').click();
});
document.getElementById('extract-input-file').addEventListener('change', (e) => {
  const file = e.target.files[0];
  if (file) readExtractFile(file);
});
...
document.getElementById('extract-download-button').addEventListener('click', () => {
...
// File reading function
let commandsToStructureContent = '';

function readCommandsToStructureFile(file) {
  const reader = new FileReader();
  reader.onload = function(e) {
    commandsToStructureContent = e.target.result;
    document.getElementById('cmd-struct-drop-area').textContent = 'File loaded. Ready to convert to structure.';
  };
  reader.readAsText(file);
}

// Event Listeners for Commands to Structure
...
cmdStructDropArea.addEventListener('click', () => {
  document.getElementById('cmd-struct-input-file').click();
});

document.getElementById('cmd-struct-input-file').addEventListener('change', (e) => {
  const file = e.target.files[0];
  if (file) readCommandsToStructureFile(file);
});

// Convert button event listener
document.getElementById('cmd-struct-convert-button').addEventListener('click', () => {
...
// Download button event listener
document.getElementById('cmd-struct-download-button').addEventListener('click', () => {
  ...
  if (success) {
    ...
    showCmdStructValidationMessage('Failed to download structure file. Check console for errors.');
  }
});
